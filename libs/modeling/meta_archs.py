...
# from ..utils.evidential_regression_loss.loss import EvidentialLossSumOfSquares
...
#             stride=1, padding=kernel_size // 2
#         )
#         self.EvdentialReg_head = DenseNormalGamma(2304, 2304)
...
#             middle_cnt += (F.relu(self.scale[l](cur_offsets)),)
...
#         pred_offsets =(torch.cat(mu, dim=1)[pos_mask],torch.cat(v, dim=1)[pos_mask],torch.cat(alpha, dim=1)[pos_mask]
#                        ,torch.cat(beta, dim=1)[pos_mask],torch.cat(aleatoric, dim=1)[pos_mask],torch.cat(epistemic, dim=1)[pos_mask])
...
#             #     reduction='sum'
#             # )
#             reg_loss = criterion(gt_offsets, pred_offsets)
...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.train_center_sample_radius = train_cfg['center_sample_radius']
        self.train_loss_weight = train_cfg['loss_weight']
        self.Iou_Evidence_loss_weight = train_cfg['Iou_Evidence_loss_weight']
        self.train_cls_prior_prob = train_cfg['cls_prior_prob']
        self.train_dropout = train_cfg['dropout']
        self.train_droppath = train_cfg['droppath']
        self.train_label_smoothing = train_cfg['label_smoothing']
